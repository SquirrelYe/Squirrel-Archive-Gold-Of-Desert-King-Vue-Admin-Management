  ...
  methods: {
    ...
    async initStastics() {
      ...
      for (let i = 0; i < res.data.rows.length; i++) {
        ...
        // 创建资产信息
        let msg = await apis.creatOneStatistic(
          e.game_id,
          e.id,
          this.money,
          this.load
        ...
        if (i == res.data.rows.length - 1) {
          ...
          s_alert.Success(
            "参赛团队资产初始化成功!",
            `你成功初始化了${res.data.rows.length}个参赛团队资产信息！`,
            "success"
          );
        }
      }
