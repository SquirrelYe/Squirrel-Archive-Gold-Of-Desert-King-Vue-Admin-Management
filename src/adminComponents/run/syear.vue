                  ...
                  >
                    ...
                    <tbody v-if="showGameItems">
                      <tr>
                        ...
                        <td style="color: red" v-if="showGameItems.day">
                          {{ showGameItems.day.day }}
                        </td>
                        <td style="color: red">
                          {{ showGameItems.judgewhether | formatJudgeWhether }}
                        </td>
                        <td class="actions" align="center">
                          ...
                          <a
                            class="on-default remove-row font-weight-bold"
                            @click="openWhether(showGameItems)"
                            href="javascript:void(0)"
                            >开启天气显示</a
                          >
                        </td>
                ...
                <div class="table-responsive">
                  ...
                  >
                    ...
                    <tbody>
                      <tr
                        class="gradeX"
                        v-for="(item, index) in showStasticsItems"
                        :key="index"
                        :class="{
                          important: item.day.day == showGameItems.day.day,
                        }"
                      >
                        <td>{{ index }}</td>
                        <td>{{ item.name }}</td>
                        ...
                        <td v-else></td>
                        <td v-if="item.statistic">
                          {{ item.statistic.money }}
                        </td>
                        <td v-else></td>
              ...
              <div class="form-group">
                ...
                <div class="col-md-9">
                  <select class="form-control" v-model="thing">
                    <option
                      v-for="(item, index) in showThingItem"
                      :key="index"
                      :value="item.id"
                    >
...
export default {
  ...
  methods: {
    ...
    getCurrentGameInfo() {
      apis
        .getOneGameById(JSON.parse(ses.getSessionStorage("gameinfo")).id)
        .then((res) => {
    ...
    getTeamItems() {
      apis
        .getAllTeamByGameId(JSON.parse(ses.getSessionStorage("gameinfo")).id)
        .then((res) => {
    ...
    async updateStatistics() {
        ...
        apis
          ...
          .then((res) => {
            ...
            if (res.data[1]) {
              ...
              apis
                .update_number(id, team.statistic_id, this.thing, number, use)
                .then((res) => {
                  console.log(res.data);
                  apis.addOneTran(
                    this.gid,
      ...
      // 物品为 金币
        ...
        apis.getOneStastisticById(team.statistic_id).then((res) => {
          console.log(res.data);
          let onumber = this.number;
          let number = Number(res.data.money) + Number(this.number);
          apis.updateMoney(team.statistic_id, number).then((res) => {
            console.log(res.data);
            apis.addOneTran(
              this.gid,
              -1,
      ...
      // 其他物品
        ...
        apis
          .findOrCreate(team.statistic_id, this.thing, this.number, 0)
          .then((res) => {
            console.log(res.data);
            if (res.data[1]) {
              apis.addOneTran(
    ...
    async openWhether(item) {
      ...
      if (confirm("你确定要开启天气显示？")) {
        ...
        if (!judge) {
          ...
          apis.updateGameJudgeWhetherById(item.id, 1).then((res) => {
            if (res.data[0] == 1) {
    ...
    async updateDay(item) {
      ...
      let res = await apis.getAllTeamByGameId(
        JSON.parse(ses.getSessionStorage("gameinfo")).id
      );
      team = res.data.rows;
      // 第一天不统计消耗信息
      ...
      if (game.day.day < 26) {
        if (item.judgewhether == 0) {
          ...
          for (let i = 0; i < team.length; i++) {
            ...
            // 获取过滤队伍 （过滤 死亡，已到达大本营）
            console.log(curTeam);
            if (curTeam.condition != -3 && curTeam.condition != 4)
              filterTeams.push(curTeam);
          }
          // 执行资产清算
            ...
            if (condition == 1) {
              ...
              for (let i = 0; i < res.data.modules.length; i++) {
                ...
                if (e.type == 1) {
                  ...
                  await apis.updateFoodWaternumber(
                    e.statistic_module.id,
                    water
                  );
                  await apis.addOneTran(
                    this.gid,
